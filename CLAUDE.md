...
- **`SUPABASE_URL`/`SUPABASE_ANON_KEY` no topo do `<script>` são placeholders** (`https://SEU-PROJETO.supabase.co` / `SUA-ANON-KEY-AQUI`) até alguém criar o projeto no Supabase (rodar `supabase/schema.sql` no SQL Editor) e preencher com os valores reais. Sem isso, login/dados não funcionam.
